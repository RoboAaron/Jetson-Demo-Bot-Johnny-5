...
// ── Jetson motion command state ──────────────────────────────────────────────
// Received via '#VEL=<m/s>,STEER=<rad/s>' multi-char serial commands.
// The safety watchdog in JetsonBridge (Python) already zeroes these if the
// Jetson stops sending; the JETSON_STALE_MS check below provides a second
// layer of protection inside the firmware itself.
static float  jetsonSteerRads    = 0.0f;   // most-recent yaw rate command (rad/s)
static unsigned long lastJetsonMs = 0;      // millis() of last '#' command
static const float JETSON_MAX_VEL_MPS    = 2.0f;    // hard clamp
static const float JETSON_MAX_STEER_RADS = 3.14159f; // hard clamp (~180 deg/s)
static const unsigned long JETSON_STALE_MS = 600;   // zero steer if no cmd for 600 ms
...
void setup() {
  ...
  Serial.println("   • Velocity PID: 100Hz (outer loop)");
  Serial.println();
...
void loop() {
  ...
  while (Serial.available()) {
    if (Serial.peek() == '#') {
      // Multi-char Jetson motion command: #VEL=<m/s>,STEER=<rad/s>
      // Read until newline so the rest of the loop is not confused by the payload.
  ...
  if (millis() - lastVelocityPIDUpdate >= VELOCITY_PID_SAMPLE_TIME_MS) {
    ...
    // ── Integrate Jetson yaw-rate command into yawSetpoint ────────────────
    // Commands arrive at ~20 Hz so dt ≈ VELOCITY_PID_SAMPLE_TIME_MS / 1000.
    // If the Jetson has gone quiet, zero the stored steer rate so the robot
    // holds its current heading rather than continuing to spin.
    if (millis() - lastJetsonMs > JETSON_STALE_MS) {
      jetsonSteerRads = 0.0f;
    }
...
// ── Jetson motion command parser ─────────────────────────────────────────────
// Receives full line already read by the '#'-detection branch in loop().
// Format: #VEL=<float>,STEER=<float>
// VEL   → velocitySetpoint (m/s, clamped ± JETSON_MAX_VEL_MPS)
// STEER → jetsonSteerRads  (rad/s, clamped ± JETSON_MAX_STEER_RADS)
//         Integrated into yawSetpoint at 20 Hz by the velocity PID block.
  ...
  switch (cmd) {
    ...
    case 'B':
      ...
      if (loggingEnabled) {
        ...
        Serial.println("Logging is not enabled. Use 'l' to start logging first.");
      }
      break;
