...
| 4-8 | [PID State Reset on Safety Transitions + PID Logic Validation](./4-8.md) | Review | Fix `PID_v1.Initialize()` DC-bias latching on every `MANUAL→AUTOMATIC` transition across all three PIDs, audit sign/clamp/filter paths, and expose `angleInput` in the stream for filter-vs-raw diagnosis. |
...
- 2026-04-16: **4-8** InProgress → **Review** after field logs validated rearm fix; source defaults bumped (`SETTINGS_MAGIC=0xC45C4DF0`) to match proven gains `Kp=1.5,Kd=0.06,baseSetpoint=-0.84,Filt=0.25,maxCurrent=2.0` in firmware commit same day.
